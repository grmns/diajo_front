...
function VentasList() {
  ...
  const filtrarVentas = () => {
    return ventas.filter((venta) => {
      ...
      // Modificación: Considerar múltiples columnas de fecha en el filtro por mes y año
      ...
      let cumpleFiltroAnio = anioSeleccionado ? fechasVenta.some(fecha => fecha.getFullYear() === anioSeleccionado) : true;

      // Combinar todos los filtros
      return cumpleFiltroEmision && cumpleFiltroRecepcion && cumpleFiltroVencimiento && cumpleFiltroPago && cumpleFiltroMes && cumpleFiltroAnio;
    });
  };
  ...
  const confirmToast = useRef(null);  // Define confirmToast aquí
  ...
  const realizarExportacion = () => {
    const datosParaExportar = filtrarVentas().map(venta => ({
      IT: venta.IT,
      TipoDocumento: venta.tipo_documento,
      DocumentoN: venta.DCTO_N,
      RUCCliente: venta.rucCliente,
      RazónSocial: venta.razonSocial,
      TipoCliente: venta.tipoCliente,
      GrupoEconómico: venta.grupoEconomico,
      NombreGrupo: venta.nombreGrupo,
      NombreVendedor: venta.nombreVendedor,
      Moneda: venta.MONEDA,
      Importe: venta.IMPORTE,
      Plazo: venta.PLAZO,
      FechaEmisión: formatoFecha(venta.FECHA_EMISION),
      Estado: venta.estadoVenta,
      Observación: venta.OBSERVACION,
      ModoPago: venta.MODO_PAGO,
      Comisionado: venta.COMISIONADO ? 'Sí' : 'No',
      NúmeroOperación: venta.NUMERO_OPERACION
    }));
    ...
    const fechaFormato = fecha.toLocaleDateString().split('/').join('-') + '_' + fecha.toLocaleTimeString().split(':').join('-').replace(/\s/g, '');
    XLSX.writeFile(libro, `Reporte_de_Ventas_${fechaFormato}.xlsx`);
    setShowConfirmToast(false);
    toast.current.clear();  // Asegúrate de limpiar el Toast después de realizar la acción
  };

  return (
    <div className="ventas-list-container">
      <Toast ref={toast} onRemove={() => setToastActivo(false)} />
      <Toast ref={toast} onRemove={() => setShowConfirmToast(false)} />
      <div className="card">
